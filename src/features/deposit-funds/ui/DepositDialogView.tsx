...
interface DepositDialogViewProps {
  open: boolean;
  onClose: () => void;
  initialAsset?: AssetSymbol | null;
  onSubmitDeposit: (values: DepositSubmitValues) => void;
}
...
export function DepositDialogView({
  open,
  onClose,
  initialAsset,
  onSubmitDeposit,
}: DepositDialogViewProps) {
  ...
  const onSubmit = (values: DepositSubmitValues) => {
    if (!values.method || !values.asset) {
      return;
    }

    onSubmitDeposit(values);
    setStep(5);
  };
  ...
  const next = () => {
    ...
    if (step === 2 && asset && numericAmount > 0) {
      setStep(3);
      return;
    }
  ...
  const nextLabel =
    step === 3 ? 'Deposit Made' : step === 4 ? 'Confirm' : 'Continue';

  const nextEnabled =
    (step === 1 && !!method) ||
    (step === 2 && !!asset && numericAmount > 0) ||
    step === 3 ||
    (step === 4 && !!proof);
          ...
          {step === 2 && (
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                {asset && numericAmount > 0 && !isFiat(asset) && (
          ...
          {step === 5 && (
            <div className="rounded-xl border border-border bg-card p-5 text-center">
              ...
              <div className="text-sm font-medium text-foreground">
                ...
                {effectiveMethod} — proof received. Pending review.
              </div>
            </div>
          )}
        </div>
...
function BankDetails({ rail }: { rail: PaymentRail }) {
  ...
  return (
    <div className="space-y-3 rounded-xl border border-border bg-card p-4">
      ...
      <DetailRow
        label="Bank"
        value={isSepa ? 'Bank Polska SA' : 'JPMorgan Chase, N.A.'}
      />
      <DetailRow
...
function CryptoDetails({
  ...
  const address = NET_ADDR[selectedNetwork];
        ...
        <div className="flex flex-wrap gap-2">
          {networks.map((item) => (
            <Button
              ...
              type="button"
              size="sm"
              variant={item === selectedNetwork ? 'default' : 'outline'}
        ...
        <div className="flex size-32 shrink-0 items-center justify-center rounded-xl border border-border bg-background p-2">
          {qrUrl ? (
            <img
              ...
              src={qrUrl}
              alt={`Deposit QR for ${asset} on ${selectedNetwork}`}
            />
          ) : (
